...
```bash
# 查看所有模板
python pipeline/list_templates.py

# 搜索模板
python pipeline/list_templates.py --search 发票

# 查看模板占位符（含说明）
python pipeline/list_templates.py --detail invoice

# 导出PDF为高清图片
python pipeline/export_pdf.py input.pdf 3

# 自动裁剪图片用于检查
python pipeline/auto_crop.py image.png 2 2

# 打包输出为zip
python pipeline/package_output.py . output.zip
```
